...
GLOBAL_ARGS="--raft --nodiscover --gcmode=archive --networkid $NETID --raftjoinexisting $RAFTID  --rpc --rpcaddr 0.0.0.0 --rpcapi $ENABLED_API --emitcheckpoints --txpool.globalslots 20000 --txpool.globalqueue 20000"
...
PRIVATE_CONFIG=qdata/$NODENAME.ipc nohup geth --verbosity 3 --datadir qdata $GLOBAL_ARGS --rpccorsdomain "*" --raftport $RA_PORT --rpcport $R_PORT --port $W_PORT --ws --wsaddr 0.0.0.0 --wsport $WS_PORT --wsorigins '*' --wsapi $ENABLED_API --nat extip:$CURRENT_NODE_IP 2>>qdata/gethLogs/${NODENAME}.log &
